...
module.exports = function sendKey(db, config, req) {
  ...
  return db
    .task(async ts => {
      ...
      if (rows.length > 0) {
        const { email } = rows[0]
        const { key, set } = reset
          ? await setKey(ts, config, req, { email })
          : await refreshKey(ts, config, req, email)
      ...
      const { email, key } = await setKey(ts, config, req, {
        email: reqEmail,
        name
      })
