...
function access(req) {
  ...
  return req.app.locals.db
    ...
    .then(data => {
      if (
        !data ||
        (!req.session.user.hugo_admin && req.session.user.email !== data.email)
      )
        throw new AuthError()
      return {
        id,
  ...
  access(req)
    .then(({ id }) =>
      req.app.locals.db.any(
        'SELECT $1^ * FROM Nominations WHERE person_id = $2 ORDER BY category, time DESC',
        [distinct, id]
      )
    )
...
function sendNominationEmail(db, id) {
  ...
  db.task(t =>
    t.batch([
      ...
      t.any(
        `SELECT DISTINCT ON (category) * FROM Nominations
...
function nominate(req, res, next) {
  ...
  access(req)
    .then(({ id, nominator }) => {
      ...
      const values = keys
        .map(
          fn => (fn === 'nominations' ? '$(nominations)::jsonb[]' : `$(${fn})`)
        )
        .join(', ')
      return req.app.locals.db.one(
        `INSERT INTO Nominations (${keys.join(
          ', '
        )}) VALUES (${values}) RETURNING time`,
        data
      )
    })
    .then(({ time }) => {
