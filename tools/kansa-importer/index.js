...
if (loginUrl.indexOf('/login') === -1) {
  console.error(
    "Usage: node index.js 'https://api.server/login?...' [--json] [--verbose] < data.csv > skipped.json"
  )
  process.exit(1)
}
...
fetch(loginUrl, { agent })
  ...
  .then(res => {
    console.error(
      `Logged in as ${colors.green(res.email)} on ${colors.green(apiRoot)}\n`
    )
        ...
        const endLoop = setInterval(() => {
          if (++i < stage2.length) {
            ...
            setTimeout(() => {
              ...
              console.error(
                `Successfully handled ${sum.join} joins, ${
                  sum.upgrade
                } upgrades, and ${sum.paper} paper pubs sales.`
              )
              console.error(
                `Assigned member numbers for ${stage2.length} new members.`
              )
              console.error(
                `Skipped ${sum.issueSkip} due to issues and ${
                  sum.emailSkip
                } due to missing email.`
              )
              console.error(`Encountered ${sum.error} errors.`)
              console.error(
                `${paperPubs.remaining().length}/${
                  paperPubs.data.length
                } paper pubs left unhandled.`
              )
            }, 30 * 1000)
          }
        }, 50) // delay required to not saturate server
  ...
  return new Promise((resolve, reject) => {
    ...
    .then(res => {
      ...
      if (verbose)
        console.error(
          colors.gray(
            `${tag} joined as ${data.membership} on ${data.timestamp}`
          )
        )
      if (rec.supporter && rec.upgrade) {
        ...
        return POST(`people/${id}/upgrade`, upgrade).then(res => {
          ...
          if (verbose)
            console.error(
              colors.gray(
                `${tag} upgraded to ${upgrade.membership} on ${
                  upgrade.timestamp
                }`
              )
            )
    ...
    .then(() => {
      if (rec.paper_pubs_id) {
        ...
        return POST(`people/${id}/upgrade`, pp).then(res => {
          ...
          if (verbose)
            console.error(
              colors.gray(`${tag} got paper pubs on ${pp.timestamp}`)
            )
        })
      }
    })
  ...
  return res.ok
    ...
    : res.json().then(body => {
        const error = new Error(
          `${res.statusText} (${res.status}): ${body.message}`
        )
        error.url = res.url
        error.body = body
        throw error
