...
class PaperPubs {
  ...
  get(id, name) {
    ...
    switch (match.length) {
      ...
      case 1:
        const rec = match[0];
        if (rec._found) throw new Error(`Paper pubs re-match for ${name}, pp ID ${id}! ${JSON.stringify(pp)}`)
        rec._found = true;
        return rec;
      default:
        throw new Error(`Multiple matches found for ${name}, pp ID ${id}! ${JSON.stringify(match)}`);
    }
  }
