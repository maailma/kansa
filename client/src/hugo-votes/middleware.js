...
function handleSubmitVotes(dispatch, { hugoVotes }, atUnload) {
  ...
  if (!id || !signature) {
    return dispatch({
      type: 'ERROR',
      error: new Error('Voting requires id & signature!')
    })
  }
  const votes = hugoVotes
    .get('clientVotes')
    .filter(
      (catVotes, category) =>
    ...
    if (atUnload) {
      ...
      api
        ...
        .then(({ time }) => {
          ...
          if (window.ga)
            votes.forEach((_, category) =>
              ga('send', 'event', 'Vote', category)
            )
        })
        .catch(error =>
          dispatch(
            error.message === 'Client has stale data'
              ? getVotes()
              : { type: 'ERROR', error }
          )
        )
    ...
    switch (action.type) {
      ...
      case 'GET_HUGO_PACKET_SERIES_EXTRA': {
        ...
        if (!id)
          return dispatch({
            type: 'ERROR',
            error: new Error('Voter id not set!')
          })
        api
          .GET(`hugo/${id}/packet-series-extra`)
          .then(() =>
            dispatch(showMessage('Token requested, will be sent by email.'))
          )
        return
      }
      ...
      case 'GET_VOTES':
        ...
        api.GET(`hugo/${id}/votes`).then(data => {
          handleReceiveVotes(dispatch, data)
          dispatch(
            showMessage('Client votes have been refreshed from the server')
          )
        })
        break
      ...
      case 'SET_VOTER':
        if (action.id && action.signature) {
          api
            .GET(`hugo/${action.id}/votes`)
            .then(data => handleReceiveVotes(dispatch, data))
          api
            .GET(`hugo/${action.id}/packet`)
      ...
      case 'SET_VOTES':
        ...
        if (!submitBeforeUnload) {
          submitBeforeUnload = () =>
            handleSubmitVotes(() => {}, getState(), true)
