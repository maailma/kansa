...
export default class Results extends PureComponent {
  ...
  get columns() {
    ...
    const points = (
      <Column dataKey="points" key="points" label="Pts" width={40} />
    )
    const nominations = (
      <Column
        dataKey="nominations"
    ...
    const round = <Column dataKey="round" key="round" label="#" width={30} />
    const data = nominationFields(category).map(key => (
      <Column dataKey={key} flexGrow={1} key={key} label={key} width={100} />
    ))
        ...
        (list, entry, idx) =>
          list.concat(
            entry
              .get('next')
              .map(nom =>
                nom.merge(entry.getIn(['counts', nom])).set('round', 1 + idx)
              )
          ),
        results
          .map(nom =>
            nom.merge(log.last().getIn(['counts', nom])).set('round', '*')
          )
  ...
  download = () => {
    ...
    const map = Map(
      log
        ...
        .get('counts')
        .reduce(
          (data, { nominations, points }, nominee) =>
            data.concat([
              [
                nominee,
                {
