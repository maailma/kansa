...
class NominationDetails extends React.Component {
  ...
  get fields() {
    ...
    nominations.forEach(nom => {
      ...
      nominationFields(category).forEach(f => {
        fields[f] = true
      })
      seen[category] = true
    })
  ...
  removeNomination(nomination) {
    ...
    if (category === this.category && canon.get('data').equals(data)) {
      let index = nominations.findIndex(
        nom => !nom.equals(nomination) && nom.get('category') === category
      )
      if (index === -1)
        index = nominations.findIndex(nom => !nom.equals(nomination))
    ...
    return (
      <Dialog
        actions={
          <FlatButton
            icon={
              disqualified ? <DisqualifyChecked /> : <DisqualifyUnchecked />
            }
            label="Disqualified"
            labelPosition="after"
            onClick={() => {
  ...
  setCanonicalEntry = ({ index }) => {
    ...
    updateCanonEntry(
      this.canonId,
      category,
      canon.set('data', nomination.get('data'))
    )
    ...
    return canonId
      ? {
          ...
          nominations: hugoAdmin
            .get('nominations')
            .map(nominations =>
              nominations.filter(nom => nom.get('canon_id') === canonId)
            )
            .valueSeq()
            .flatten(true)
            .toList()
