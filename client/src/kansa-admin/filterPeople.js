...
export const filterTerms = [
  [
    '_some terms_',
    'Match both _some_ and _terms_, possibly in different fields'
  ],
...
function getMemberFilter(filter) {
  const mapHasValue = (map, cmp) =>
    map.valueSeq().some(value => {
    ...
    const compare = (key, cmp) => {
      switch (key) {
        ...
        case 'is':
          if (cmp === 'public')
            return (
              member.has('public_first_name') || member.has('public_last_name')
            )
          return (
            member
              .get('membership', '')
...
export default function filterPeople(people, filterSrc) {
  const filter =
    cache[filterSrc] || (src => (cache[src] = parseFilter(src)))(filterSrc)
