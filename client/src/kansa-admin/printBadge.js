...
export default function printBadge (server, printer, member) {
  const {
    badge_name, badge_subtitle, country, legal_name,
    member_number, membership, public_first_name, public_last_name
  } = member.toJS()
  const public_name = [public_first_name, public_last_name].filter(n => n).join(' ').trim()
  const [FirstName, Surname] = splitNameInTwain(badge_name || public_name || legal_name)
