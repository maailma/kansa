...
class Voter extends Component {
  ...
  componentWillReceiveProps({ api, member }) {
    if (member && !member.equals(this.props.member)) {
      ...
      api
        .GET(`siteselect/voters/${member.get('id')}`)
        .then(tokens =>
          this.setState({
      ...
      api
        .GET(`people/${member.get('id')}/prev-names`)
  ...
  vote = () => {
    ...
    api
      .POST(`siteselect/voters/${member.get('id')}`, {
  ...
  renderPastNames = () =>
    this.state.past_names.map(({ prev_legal_name, time_from, time_to }, i) => {
      const t0 = time_from
        ? new Date(time_from).toISOString().substr(0, 10)
        : '???'
      const t1 = time_to ? new Date(time_to).toISOString().substr(0, 10) : '???'
      return (
        <p key={i}>
          Previous name: <b>{prev_legal_name}</b> ({t0} - {t1})
        </p>
      )
    })
  ...
  render() {
    ...
    const { legal_name, email, membership, member_number } = member
      ? member.toJS()
      : {}
    return (
      <Dialog
