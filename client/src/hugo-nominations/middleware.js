...
function submitNominations(
  ...
  if (!list)
    throw new Error(
      `Nominations for category ${JSON.stringify(category)} not found!`
    )
  if (list.equals(nominations.getIn([category, 'serverData']))) return

  const fields = nominationFields(category)
  if (
    list.some(nomination =>
      nomination.some((_, field) => fields.indexOf(field) === -1)
    )
  ) {
    throw new Error(
      `Unknown key in nomination data: ${JSON.stringify(list.toJS())}`
    )
  }

  api
    .POST(`hugo/${person}/nominate`, {
      category,
      nominations: list.toJS(),
      signature
    })
    .then(res => {
      dispatch(getNominations(res))
      if (window.ga)
        ga('send', 'event', 'Nominations', 'set', category, list.size)
    })
    .catch(error => dispatch({ type: 'ERROR', error }))
}
