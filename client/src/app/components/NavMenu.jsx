...
const singleWithKey = (currentMember, otherMembers, key) =>
  !currentMember || !currentMember.get(key)
    ? false
    : otherMembers
      ? otherMembers.every(m => !m.get(key))
      : true
...
const NavMenu = ({ currentMember, handleNav, otherMembers }) => {
  ...
  if (memberItems.length) {
    memberItems.push(
      <Divider key="div" inset style={{ marginTop: 8, marginBottom: 8 }} />
    )
  }

  return (
    <SelectableList onChange={(ev, val) => handleNav(val)}>
      {memberItems}
      <ListItem
        leftIcon={<People />}
